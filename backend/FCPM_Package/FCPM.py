...
class FCPM:
    ...
    # def extract_prefix_VS(self, df, outcome_class):
    #     """
    #     Compute how many distinct entities had each prefix
    #     at least once.

    #     Args:
    #         df (pd.DataFrame): Must include 'EntityID' in the form '12_3_2' etc.
    #         prefix_feature_map (dict): Mapping of prefix binary columns to their respective feature lists.

    #     Returns:
    #         tuple:
    #             - dict: Mapping each prefix column -> the count of top-level entities that had it == 1.
    #             - int: The total number of unique top-level entities.
    #     """
    #     unique_entities = df['EntityID'].nunique()
    #     return unique_entities / self.total_entities
    #     # if outcome_class == 0:
    #     #     return unique_entities / self.total_entities_controls
    #     # else:
    #     #     return unique_entities / self.total_entities_cases

    # def fit_distributions(self,prefix_name, outcome_class, plots_dir=None, create_plots=False):
    #     """
    #     Fit distributions for the specified class.

    #     Args:
    #         class_id (int): The class ID (0 or 1).

    #     Returns:
    #         dict: A dictionary where keys are duration feature names and values are distribution details.
    #     """
    #     durations = self.prefixes_durations_class_0 if outcome_class == 0 else self.prefixes_durations_class_1

    #     fitted_distributions = {}

    #     features_dict = durations.get(prefix_name, {})

    #     fitted_distributions = {}
    #     for feature_name, feature_durations_list in features_dict.items():
    #         print(f"Fitting distribution for feature: {feature_name} in prefix: {prefix_name}")
    #         if feature_durations_list:
    #             # --- Pass plot parameters, prefix name, and feature name to _fit_feature ---
    #             fit_result = self._fit_feature(
    #                 feature_durations_list,
    #                 prefix_name=prefix_name,      # Pass prefix name
    #                 feature_name=feature_name,    # Pass feature name
    #                 outcome_class=outcome_class,  # Pass outcome class
    #                 plot_dir=plots_dir,          # Pass plots directory
    #                 create_plots=create_plots     # Pass flag
    #             )
    #             if fit_result: # Check if fitting was successful
    #                 fitted_distributions[feature_name] = fit_result
    #             else:
    #                 fitted_distributions[feature_name] = None

    #         else:
    #             print(f"  -> No duration values for this feature. Skipping.")
    #             fitted_distributions[feature_name] = None # Store None if no data

    #     # Store results in appropriate class storage
    #     if outcome_class == 0:
    #         self.class_0_dist[prefix_name] = fitted_distributions
    #     elif outcome_class == 1:
    #         self.class_1_dist[prefix_name] = fitted_distributions
        ...
        with warnings.catch_warnings():
            ...
            try:  # Add a try-except block for robustness during fitting itself
                ...
                if len(params) >= 2:
                    ...
                    logging.warning(
                        f"    - SSE: Unexpected number of parameters {len(params)} from {dist.name}.fit(). Skipping.")
                ...
                if len(x_edges) < 2 or np.all(y_hist == 0):
                    logging.warning(
                        f" - SSE: Histogram for {dist.name} resulted in < 2 edges or all zero counts with {number_of_bins} bins. Cannot calculate SSE.")
                    return np.inf, None, None
                ...
                if np.isfinite(sse) and sse >= 0:
                    ...
                    logging.warning(
                        f"    - SSE: Invalid SSE value ({sse}) calculated for {dist.name}. Returning Inf score.")
    ...
    def _fit_feature(self, durations, feature_name, outcome_class, plot_dir=None, create_plots=False):
        ...
        if data.empty or len(data) < 2:
            logging.warning(
                f"  - Feature '{feature_name}': No positive duration data or < 2 points ({len(data)}) after filtering. Cannot fit.")
            return None
        ...
        if best_distribution is None or best_params is None:
            logging.warning(
                f"  - Feature '{feature_name}': No suitable distribution found using method '{self.fit_method}'.")
            return None
        ...
        if create_plots and plot_dir:
            ...
            try:
                ...
                ax.hist(data, bins=plot_bins, density=True, alpha=0.6, color='skyblue',
                        label=f'Data Histogram (Bins={plot_bins})')
    ...
    def fit(self, fm_dir_cls1, fm_dir_cls0, cases_data_path, controls_data_path, plots_dir=None):
        ...
        for prefix_fm_file_path in glob(os.path.join(fm_dir_cls1, "*.csv")):
            if basename(prefix_fm_file_path).startswith("merged_df") or basename(prefix_fm_file_path).startswith(
                    "TTE_") or basename(prefix_fm_file_path).startswith("1_"):
                continue
        ...
        for prefix_fm_file_path in glob(os.path.join(fm_dir_cls0, "*.csv")):
            if basename(prefix_fm_file_path).startswith("merged_df") or basename(prefix_fm_file_path).startswith(
                    "TTE_") or basename(prefix_fm_file_path).startswith("1_"):
                continue
            number_of_instances, prefix_df, prefix_name = self._load_csv(prefix_fm_file_path)
        ...
        self.prefixes_support_cases = {prefix: support / total_instances for prefix, support in
                                       self.prefixes_support_cases.items()}
        self.prefixes_support_controls = {prefix: support / total_instances for prefix, support in
                                          self.prefixes_support_controls.items()}
        ...
        for feature_name in self.durations_cases.keys():
            fitted_dist_info = self._fit_feature(self.durations_cases[feature_name], feature_name, 1, plots_dir,
                                                 create_plots)
            self.cases_dist[feature_name] = fitted_dist_info
        for feature_name in self.durations_controls.keys():
            fitted_dist_info = self._fit_feature(self.durations_controls[feature_name], feature_name, 0, plots_dir,
                                                 create_plots)
            self.controls_dist[feature_name] = fitted_dist_info

    # def fit_old(self, fm_dir, outcome_class, plots_dir = None):
    #     """
    #     Train the FCPM model by fitting distributions for both classes.

    #     CAN BE PHARALLELIZED FOR EACH CLASS!!!
    #     #TODO: I need to get from the detection as parameter the total number of entities
    #     """
    #     create_plots = plots_dir is not None
    #     if create_plots:
    #         os.makedirs(plots_dir, exist_ok=True) # Create dir if it doesn't exist

    #     for prefix_fm_file_path in glob(os.path.join(fm_dir, "*.csv")):
    #         if basename(prefix_fm_file_path).startswith("1_") or basename(prefix_fm_file_path).startswith("TTE_"):
    #             continue
    #         prefix_fm, prefix_name = self._load_csv(prefix_fm_file_path)
    #         if outcome_class==1:
    #             print(f'prefix name is: {prefix_name}')

    #         if prefix_name is not None:
    #             extracted_durations = self.extract_durations(prefix_name, prefix_fm)
    #             extracted_support = self.extract_prefix_VS(prefix_fm, outcome_class)

    #             if outcome_class == 0:
    #                 self.prefixes_durations_class_0[prefix_name] = extracted_durations[prefix_name]
    #                 self.prefixesVS_class_0[prefix_name] = extracted_support
    #             else:# outcome_class is 1
    #                 self.prefixes_durations_class_1[prefix_name] = extracted_durations[prefix_name]
    #                 self.prefixesVS_class_1[prefix_name] = extracted_support

    #             # --- Call fit_distributions with plot parameters ---
    ...
    #             print(f"Model trained successfully for: {prefix_name}.")

    def predict(self, timestamp, entitiy_df_per_timestamp, epsilon=1):
    ...
    # def _get_censoring_probability(self, cdf_series, value):
    #     """
    #     Compute the survival probability = 1 - CDF(value),
    #     where CDF is stored in cdf_series (index -> CDF(x)).

    #     This is used for censoring, i.e. the 'event' has not completed by 'value'.
    #     """
    #     cdf_val = self._lookup_cdf(cdf_series, value)
    #     epsilon_clamp = 1e-9
    #     cdf_val = max(epsilon_clamp, min(1.0 - epsilon_clamp, cdf_val))
    #     prob = 1.0 - cdf_val
    #     return prob
    ...
    # def _get_probability_from_cdf(self, dist_info, value, epsilon):
    #     """
    #     Calculate probability P(value - epsilon < X <= value + epsilon)
    #     using the THEORETICAL CDF of the fitted distribution.
    ...
    #     # Calculate probability directly using the theoretical CDF difference
    #     # P(a < X <= b) = CDF(b) - CDF(a)
    #     cdf_up = dist.cdf(value + epsilon, *params)
    #     cdf_down = dist.cdf(value - epsilon, *params)
    #     prob = cdf_up - cdf_down

    #     # Clamp probability to [0, 1] due to potential float inaccuracies
    ...
    # def _get_censoring_probability(self, dist_info, value):
    #     """
    #     Compute the survival probability P(X > value) = 1 - CDF(value), using the
    #     THEORETICAL distribution's CDF or Survival Function (SF).
    ...
    #     # Calculate theoretical survival probability P(X > value)
    #     # Using the Survival Function (SF = 1 - CDF) is often more numerically stable
    #     if hasattr(dist, 'sf'): # Check if the distribution has a survival function method
    #             prob = dist.sf(value, *params) # sf directly calculates P(X > value)
    #     else: # Fallback if sf method doesn't exist
    #             cdf_val = dist.cdf(value, *params)
    #             prob = 1.0 - cdf_val
    ...
    # def _get_probability_from_cdf(self, cdf_series, value, epsilon):
    #     """
    #     Approximate PDF by using the difference:
    #       CDF(value + epsilon) - CDF(value - epsilon).
    #     If outside range, clamp to 0 or 1.

    #     Args:
    #         cdf_series (pd.Series): The discrete CDF stored in dist_info["cdf"].
    #         value (float): Observed duration value.
    #         epsilon (float): Smoothing half-range.

    #     Returns:
    #         float: Probability in [0..1].
    #     """
    #     # cdf_series is indexed by x-values, so we find:
    #     # cdf_up = cdf(value + epsilon)
    #     # cdf_down = cdf(value - epsilon)

    #     epsilon_clamp = 1e-9

    #     # 1) If value < min index:
    #     min_x = cdf_series.index[0]
    #     max_x = cdf_series.index[-1]

    #     # if value + epsilon < min_x or len(cdf_series) == 0:
    #     #     return 0.0
    #     # if value - epsilon > max_x:
    #     #     return 0.0

    #     cdf_up = self._lookup_cdf(cdf_series, value + epsilon)
    #     cdf_down = self._lookup_cdf(cdf_series, value - epsilon)

    #     # --- Apply clamping to looked-up values ---
    #     cdf_up = max(epsilon_clamp, min(1.0 - epsilon_clamp, cdf_up))
    #     cdf_down = max(epsilon_clamp, min(1.0 - epsilon_clamp, cdf_down))

    #     prob = cdf_up - cdf_down

    #     # If negative or other edge-case, clamp to [0..1]
    #     prob = max(0.0, min(1.0, prob))
...
#     fm_0_path = r"/sise/robertmo-group/Eldar/CPM_result_ahe_200525/ahe_small_paa120/fold_1/abs_b=2-equal-frequency-ig=1/mine_e=0-mg=9999-mvs=0_dot_7-rel=7-sf=True/feature_matrix/tirp_3-7_8_999_1_0_0/class0/"
#     fm_1_path = r"/sise/robertmo-group/Eldar/CPM_result_ahe_200525/ahe_small_paa120/fold_1/abs_b=2-equal-frequency-ig=1/mine_e=0-mg=9999-mvs=0_dot_7-rel=7-sf=True/feature_matrix/tirp_3-7_8_999_1_0_0/class1/"
#     cases_data_path = r"/sise/robertmo-group/Eldar/CPM_result_ahe_200525/ahe_small_paa120/fold_1/abs_b=2-equal-frequency-ig=1/mine_e=0-mg=9999-mvs=0_dot_7-rel=7-sf=True/feature_matrix/tirp_3-7_8_999_1_0_0/cases.csv"
#     controls_data_path = r"/sise/robertmo-group/Eldar/CPM_result_ahe_200525/ahe_small_paa120/fold_1/abs_b=2-equal-frequency-ig=1/mine_e=0-mg=9999-mvs=0_dot_7-rel=7-sf=True/feature_matrix/tirp_3-7_8_999_1_0_0/controls.csv"

#     fit_method = "sse"
#     model = FCPM(fit_method=fit_method)
#     debug_plots_dir = f"/sise/robertmo-group/Eldar/projects/CPM_Framework/FCPM_Package/debug_plots_{fit_method}"
#     # debug_plots_dir1 = f"/sise/robertmo-group/Eldar/projects/CPM_Framework/FCPM_Package/debug_plots_{fit_method}_class1"
#     model.fit(fm_1_path, fm_0_path, cases_data_path, controls_data_path, debug_plots_dir)
