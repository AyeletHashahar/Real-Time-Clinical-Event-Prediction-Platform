import pandas as pd
from const import (DatasetColumns, EntityClassRelationsColumns, SymbolicTimeSeriesColumns,
                                            TimeIntervalsColumns, StatesColumns, TemporalAbstractionParamsColumns,
...
class DataframesGenerator:
    ...
    @staticmethod
    def generate_discretization_property(properties, timestamps, properties_values, states):
        return pd.DataFrame({
            DatasetColumns.TemporalPropertyID: properties,
            DatasetColumns.TimeStamp: timestamps,
            DatasetColumns.TemporalPropertyValue: properties_values,
            StatesColumns.StateID: states
        })

    @staticmethod
    def generate_empty_discretization_property():
        return DataframesGenerator.generate_discretization_property(
            [], [], [], []
        )
    ...
    def generate_entity_class_relations(entities, classes):
        return pd.DataFrame({
            ...
            EntityClassRelationsColumns.GroupID: classes
        })
