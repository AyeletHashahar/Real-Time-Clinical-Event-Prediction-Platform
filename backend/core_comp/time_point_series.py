...
class TimePointSeries:
    ...
    def are_end_tieps_closed_their_start_tieps(self, time_point: TimePoint) -> bool:
        # this function checks if an end tiep ends the last start tiep with the same state id of the prev instance
    ...
    def _create_time_point_series(tiep_series: TiepSeries) -> list[TimePoint]:
        ...
        for tiep in tiep_series.get_tiep_list():
            if tiep.get_time() not in time_point_series:
                time_point_series[tiep.get_time()] = TimePoint(time=tiep.get_time())
            time_point_series[tiep.get_time()].add_tiep(tiep)
