...
class TIRP:
    ...
    def __init__(self, stis: list[int], temp_rels: list[str], states_table, vs: float = 0, hs: float = 1):
        """
        ...
        Note: The char 'X' represents empty since we do not need the reverse relation between states.
    ...
    def get_property_id(self, state_id: int) -> int:
        """
        This function gets the variable id of the symbol id
        :param state_id: the symbol id
        :return: the property id
        """
        if not state_id == const.EVENT_INDEX:
            row = self.state_table[self.state_table[const.StatesColumns.StateID] == state_id]
            property_id = row[const.StatesColumns.TemporalPropertyID].iloc[0]
        else:
            property_id = 0
    ...
    def _get_tiep_order(self) -> list[list[Tiep]]:
        ...
        :param states_list: a list of relations, in this format = ['A', 'B', 'C', 'D']
        :return: a list of all the tieps of the given pattern,
        in this format : [['A+'], ['B+'], ['A-', 'C+'], ['B-', 'C-']]
        """
        tieps_list: list = []
        tiep_index = 0
        for state_i, state_id in enumerate(self.stis):
            # adding the starting tiep
            starting_tiep = Tiep(time=-1, tiep_type=const.START_TIEP, state_id=state_id, state_inst_id=state_i,
                                 property_id=self.get_property_id(state_id), tiep_inst_id=tiep_index, dummy=True)
            tieps_list.append(starting_tiep)
            tiep_index += 1

            # adding the ending tiep
            ending_tiep = Tiep(time=-1, tiep_type=const.END_TIEP, state_id=state_id, state_inst_id=state_i,
                               property_id=self.get_property_id(state_id), tiep_inst_id=tiep_index, dummy=True)
        ...
        sorted_tieps = self._merge_co_occur_tieps(co_occur_tiep_ids=o_occur_tieps,
                                                  tieps=tieps_list)
        return sorted_tieps

    def _merge_co_occur_tieps(self, co_occur_tiep_ids, tieps):
        """
        Merges co-occurring TIEPs without assuming they are adjacent in the list.
        """
        # Map from TIEP ID to its group index
        id_to_group_index = {}
        for group_index, ids in enumerate(co_occur_tiep_ids):
            for _id in ids:
                id_to_group_index[_id] = group_index

        # Prepare empty groups
        groups = [[] for _ in range(len(co_occur_tiep_ids))]
        merged_output = []  # Final result in chronological order
        seen_groups = set()  # Keep track of which groups we've already added

        for t in tieps:  # Iterate in the predefined order (e.g., sorted)
            group_id = id_to_group_index.get(t.get_tiep_instance_id())
            if group_id is not None:
                groups[group_id].append(t)
                # If it's the first time we encounter this group, add it to the output
                if group_id not in seen_groups:
                    merged_output.append(groups[group_id])
                    seen_groups.add(group_id)
            else:
                # This TIEP is not part of any co-occurrence group
                merged_output.append([t])

        return merged_output

    @staticmethod
    def find_sublist_with_item(list_of_lists: list[list], item):
        """
    ...
    def _sort_tieps_by_stis_temp_rels(self, earlier_tiep: Tiep, later_tiep: Tiep):
        ...
        if earlier_sti_index > later_sti_index:
            ...
            if earlier_tiep_type == later_tiep_type:  # if it is the same tiep (e.g., A+ and A+)
                ...
                result = 1  # if the earlier is - and the second is later is + (e.g., A- and A+)
        else:  # different state indexes (e.g., A and a different A)
