...
export default function PatternIntervals({ patternIndex, eventIndex, graphId, webSocketService }) {
  ...
  // Add a flag to track chart initialization
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Get all patterns from DataManagementService, passing the eventIndex
        const patternsArray = await DataManagementService.fetchAllPatterns(eventIndex);
...
useEffect(() => {
  if (patternData && relatedTemporalPropertyIds.length > 0) {
    ...
    if (!chartRef.current && !chartInitializedRef.current) {
      ...
      initializeChart(categories, stateIds);
      chartInitializedRef.current = true;
    }

    // Initialize lastDataTimeRef to 0 if not already set
    if (lastDataTimeRef.current === null) {
      lastDataTimeRef.current = 0;
      lastDataReceivedTimeRef.current = 0;
    }
    ...
    const processIntervalDeletions = (intervalsToFix) => {
      ...
      matchingPropertyIds.forEach(propertyId => {
        ...
        intervalSeries.data.forEach((point) => {
          ...
          if (point.color === 'rgb(207, 207, 207)' && point.y === stateIndex) {
    ...
    // Set interval to handle gaps in data only if chart is initialized
    ...
    intervalIdRef.current = setInterval(() => {
      const now = lastDataTimeRef.current;
      if (now !== null && (now - lastDataReceivedTimeRef.current) < 1000) {
        lastDataTimeRef.current += 1000;
        ...
        clearInterval(intervalIdRef.current);
        intervalIdRef.current = null;
        //console.log('No data received for 5 seconds. Stopping chart progression.');
      }
    }, 1000);
...
}, [patternData, webSocketService, relatedTemporalPropertyIds, eventIndex]);
...
useEffect(() => {
  ...
  const initializeChart = (categories, stateIds) => {
    const initialData = stateIds.map((stateId, index) => ({
      x: 0,
      x2: 0, // No interval yet
      y: index,
      name: categories[index],
      color: 'transparent', // Start with transparent intervals
      dataLabels: {
        enabled: false,
        format: '{point.name}'
      }
  ...
  const handleNoNewData = (timestamp) => {
    ...
    // For each state, add a blank (transparent) placeholder point for the current time
    Object.keys(stateIdToYIndexMapRef.current).forEach((stateId) => {
      ...
      // Add a transparent point to represent no data for this interval
    ...
    chart.series[0].addPoint(
      {
        ...
        y: 0, // Use 0 for pattern intervals in this component
    ...
    <div style={{ 
      ...
      <div
        ...
        style={{
          height: '230px',
          width: '100%',
          border: '2px solid #E0E0E0', //#7F7F7F',
          borderRadius: '4px',
          padding: '5px',
          margin: '0',
          marginBottom: '0px',
