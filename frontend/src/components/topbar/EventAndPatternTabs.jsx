...
//   // Set up event active listeners for all events
//   useEffect(() => {
//     const cleanupEventActiveListeners = () => {
//       activeListenersRef.current.forEach(({ eventId, callback }) => {
//         DataUpdateService.removeEventActiveListener(eventId, callback);
//       });
//       activeListenersRef.current = [];
//     };

//     const setupEventActiveListeners = () => {
//       // Cleanup existing listeners first
//       cleanupEventActiveListeners();
...
//         newListeners.push({ eventId: event.id, callback: handleEventActiveUpdate });
//       });
    ...
    const setupListeners = () => {
      ...
      events.forEach(event => {
        ...
        const handleEventOccurredUpdate = (occurred) => {
          setEventOccurredStates(prev => ({
            ...prev,
            [event.id]: toServerFlag(occurred)
          }));
        };
        ...
        >
          ...
          <Tab 
            ...
            sx={{
              fontSize: '14px',
              fontWeight: !rawView && selectedEvent === MAIN_TAB_ID ? 'bold' : 'normal',
              // The next 4 lines control the color of main tab when not selected
              '&:not(.Mui-selected)': {
                backgroundColor: '#cbd5e0', // Darker background when not selected
                borderTopLeftRadius: '8px',
                borderTopRightRadius: '8px',
              }
            }}
          ...
          {events.map((event) => {
            ...
            const probabilityColor = getProbabilityColor(probabilityValue);
            ...
            const tabLabel = isSelectedEvent ? (
              <div style={{ 
                ...
                <span style={{ fontSize: '14px' }}>{event.name.replace(/_/g, ' ')}</span>
                <span style={{ 
                  fontSize: '12px', 
                  color: isEventOccurred ? 'rgb(150, 0, 0)' : probabilityColor,
                  fontWeight: isEventOccurred ? 'bold' : 'normal',
                  marginTop: '1px'
                }}>
                  {isEventOccurred ? 'Active' : formattedProb}
                </span>
              </div>
            ) : event.name.replace(/_/g, ' '); // Just show event name for non-selected tabs
            ...
            return (
              <Tab 
                ...
                sx={{
                  fontSize: '14px',
                  fontWeight: !rawView && selectedEvent === event.id ? 'bold' : (isEventOccurred ? 'bold' : 'normal'),
                  color: isEventOccurred ? 'rgb(150, 0, 0) !important' : '#7F7F7F',
                  '&.Mui-selected': {
                    color: isEventOccurred ? 'rgb(150, 0, 0) !important' : '#000000',
                    fontWeight: 'bold',
                    backgroundColor: '#FFFFFF',
                    borderTopLeftRadius: '8px',
                    borderTopRightRadius: '8px',
